package com.sulake.core.window.services
{
   ...
   public class GestureAgentService implements IGestureAgentService, IDisposable
   {
      ...
      public function begin(param1:IWindow, param2:Function, param3:uint, param4:int, param5:int) : IWindow
      {
         ...
         {
            this._window = param1;
            this._window.addEventListener(WindowEvent.const_319,this.clientWindowDestroyed);
            this.var_268 = param2;
            this._working = true;
      ...
      protected function operate(param1:TimerEvent) : void
      {
         this.var_1224 *= 0.75;
         this.var_1225 *= 0.75;
         if(Math.abs(this.var_1224) <= 1 && Math.abs(this.var_1225) <= 1)
         {
            this.end(this._window);
         }
         else if(this.var_268 != null)
         {
            this.var_268(this.var_1224,this.var_1225);
         }
      }
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(this._working)
         {
            if(this._window == param1)
            {
               ...
               {
                  this._window.removeEventListener(WindowEvent.const_319,this.clientWindowDestroyed);
               }
               this._window = null;
               this._working = false;
            }
