package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      {
         return this.var_432.length + this.var_271.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_271.length < const_1520 && this.var_432.length > 0)
            {
               _loc1_ = this.var_432.shift();
               this.var_271.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_432.indexOf(loader);
            if(index > -1)
            {
               this.var_432.splice(index,1);
            }
            index = this.var_271.indexOf(loader);
            if(index > -1)
            {
               this.var_271.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_432)
            {
               if(param2)
               {
