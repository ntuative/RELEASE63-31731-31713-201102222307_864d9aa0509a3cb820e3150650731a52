package com.sulake.habbo.tracking
{
   ...
   public class LatencyTracker
   {
      ...
      {
         if(this.var_167 == null || this._communication == null || this._connection == null)
         {
            return;
         }
         this.var_1512 = int(this.var_167.getKey("latencytest.interval"));
         this.var_1513 = int(this.var_167.getKey("latencytest.report.index"));
         this.var_2036 = int(this.var_167.getKey("latencytest.report.delta"));
      ...
      public function update(param1:uint, param2:int) : void
      {
         ...
         if(param2 - this.var_1514 > this.var_1512)
         {
            this.testLatency();
         }
      }
      ...
      {
         this.var_1514 = getTimer();
         this.var_544.add(this.var_1259,this.var_1514);
         this._connection.send(new LatencyPingRequestMessageComposer(this.var_1259));
         ++this.var_1259;
      }
      ...
      private function onPingResponse(param1:IMessageEvent) : void
      {
         ...
         var _loc10_:int = 0;
         var _loc11_:* = null;
         if(this.var_544 == null || this.var_205 == null)
         {
            return;
         }
         var _loc2_:LatencyPingResponseMessageParser = (param1 as LatencyPingResponseMessageEvent).getParser();
         var _loc3_:int = this.var_544.getValue(_loc2_.requestId);
         this.var_544.remove(_loc2_.requestId);
         var _loc4_:int = getTimer() - _loc3_;
         this.var_205.push(_loc4_);
         if(this.var_205.length == this.var_1513 && this.var_1513 > 0)
         {
            _loc5_ = 0;
            _loc6_ = 0;
            _loc7_ = 0;
            ...
            if(Math.abs(_loc9_ - this.var_1515) > this.var_2036 || this.var_1515 == 0)
            {
