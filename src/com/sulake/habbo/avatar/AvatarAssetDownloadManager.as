package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_400:AssetLoaderStruct;
      ...
      private const const_1967:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_400 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_882 = new Timer(this.const_1967,1);
         this.var_882.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_400)
         {
            this.var_400.addEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_400.addEventListener(AssetLoaderEvent.const_44,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_400)
         {
            this.var_400.removeEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_400.removeEventListener(AssetLoaderEvent.const_44,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         else
         {
            ...
            if(this.var_1321.indexOf("?") > 0)
            {
               _loc2_ = this.var_1321 + "&retry=" + this.var_881;
            }
            else
            {
               _loc2_ = this.var_1321 + "?retry=" + this.var_881;
            ...
            this.var_400 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2261,this._assets);
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1634)
         {
            this.var_1100.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_880)
         {
            _loc7_ = true;
            _loc4_ = this.var_880[_loc3_];
         ...
         {
            delete this.var_880[_loc3_];
         }
         while(_loc6_ < this.var_707.length)
         {
            _loc5_ = this.var_707[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_707.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_882.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_708.length > 0 && this.var_707.length < this.const_1520)
         {
            _loc1_ = this.var_708[0];
            _loc1_.startDownloading();
            this.var_707.push(this.var_708.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_708.indexOf(param1) == -1 && this.var_707.indexOf(param1) == -1)
         {
            this.var_708.push(param1);
            this.processPending();
         }
      }
