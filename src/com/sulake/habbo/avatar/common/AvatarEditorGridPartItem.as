package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private const const_1965:int = 13421772;
      
      private const const_1964:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_424 = this.var_65.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_650 != null && !this.var_1804)
            {
               _loc3_ = this.var_650;
            }
            else
            ...
            if(this.var_755)
            {
               switch(this.var_755.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_40:
                     _loc2_.style = HabboIconType.const_888;
                     break;
         ...
         if(this.isSelected)
         {
            this._window.color = this.const_1965;
         }
         else
         {
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_424.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_424.isFigureReady(_loc1_))
         {
            this.var_424.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_329[_loc6_] + "_" + FigureData.const_154;
               _loc4_ = this.var_424.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         _loc1_ = new BitmapData(this.var_615.width,this.var_615.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_615.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_615.y;
