package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            this.var_1863 = param1.type;
            this.var_1436 = false;
            this.var_1191 = param1.position;
         }
         else
         {
            this.var_1436 = true;
         }
         this.var_2609 = param1.hasHabboClub;
         this.var_1862 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",this.var_1191.toString());
         localizations.registerParameter("room.queue.position.hc","position",this.var_1191.toString());
         localizations.registerParameter("room.queue.spectator.position","position",this.var_1191.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",this.var_1191.toString());
         this.showInterface();
      }
      ...
      private function showInterface() : void
      {
         ...
         {
            switch(this.var_1863)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_358:
                  _loc1_.caption = !!this.var_1862 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = this.var_1436;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_543:
                  _loc1_.caption = !!this.var_1862 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_628);
      ...
      {
         messageListener.processWidgetMessage(new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_626));
      }
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(this.var_1863 == RoomWidgetRoomQueueUpdateEvent.const_358)
         {
