package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class ColourGridCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_1856 = _loc1_.content as XML;
      ...
      private function onAvailableColours(param1:CatalogWidgetColoursEvent) : void
      {
         ...
         this.var_624 = _loc3_.content as BitmapData;
      ...
      private function populateColourGrid() : void
      {
         ...
         var _loc12_:int = 0;
         var _loc13_:int = 0;
         Logger.log("[ColourGridCatalogWidget] Display colors: " + [this.var_960.length,this.var_430.numGridItems]);
         this.var_430.destroyGridItems();
         this.var_356 = null;
         for each(_loc1_ in this.var_960)
         {
            if(_loc1_.length > 0)
            {
               _loc2_ = page.viewer.catalog.windowManager.buildFromXML(this.var_1856) as IWindowContainer;
               _loc2_.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.onClick);
               _loc2_.background = true;
               _loc2_.color = 4294967295;
               _loc2_.width = this.var_624.width;
               _loc2_.height = this.var_624.height;
               this.var_430.addGridItem(_loc2_);
               _loc3_ = _loc2_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
               if(_loc3_ != null)
               {
                  _loc3_.bitmap = new BitmapData(this.var_624.width,this.var_624.height,true,0);
                  _loc3_.bitmap.copyPixels(this.var_624,this.var_624.rect,new Point(0,0));
               }
               ...
               if(_loc5_ != null)
               {
                  _loc5_.bitmap = new BitmapData(this.var_828.width,this.var_828.height,true,16777215);
                  _loc5_.bitmap.copyPixels(this.var_828,this.var_828.rect,new Point(0,0),null,null,true);
